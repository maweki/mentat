...
fn set_user_version(conn: &rusqlite::Connection, version: i32) -> Result<()> {
    conn.execute(&format!("PRAGMA user_version = {}", version), &[])
        .context(SqlErrorKind::CouldNotSetVersionPragma)?;
    Ok(())
}
...
fn get_user_version(conn: &rusqlite::Connection) -> Result<i32> {
    let v = conn.query_row("PRAGMA user_version", &[], |row| {
        row.get(0)
    }).context(SqlErrorKind::CouldNotGetVersionPragma)?;
    Ok(v)
}
...
fn search(conn: &rusqlite::Connection) -> Result<()> {
    ...
    stmt.execute(&[]).context(SqlErrorKind::CouldNotSearch)?;
    Ok(())
}
...
fn insert_transaction(conn: &rusqlite::Connection, tx: Entid) -> Result<()> {
    ...
    let mut stmt = conn.prepare_cached(s)?;
    stmt.execute(&[&tx]).context(SqlErrorKind::TxInsertFailedToAddMissingDatoms)?;
    ...
    let mut stmt = conn.prepare_cached(s)?;
    stmt.execute(&[&tx]).context(SqlErrorKind::TxInsertFailedToRetractDatoms)?;
...
fn update_datoms(conn: &rusqlite::Connection, tx: Entid) -> Result<()> {
    ...
    let mut stmt = conn.prepare_cached(s)?;
    stmt.execute(&[]).context(SqlErrorKind::DatomsUpdateFailedToRetract)?;
    ...
    let mut stmt = conn.prepare_cached(&s)?;
    stmt.execute(&[&tx]).context(SqlErrorKind::DatomsUpdateFailedToAdd)?;
    Ok(())
}
...
impl MentatStoring for rusqlite::Connection {
    ...
    fn begin_tx_application(&self) -> Result<()> {
        ...
        for statement in &statements {
            ...
            stmt.execute(&[]).context(SqlErrorKind::FailedToCreateTempTables)?;
        }
    ...
    fn insert_non_fts_searches<'a>(&self, entities: &'a [ReducedEntity<'a>], search_type: SearchType) -> Result<()> {
        ...
        let results: Result<Vec<()>> = chunks.into_iter().map(|chunk| -> Result<()> {
            ...
            stmt.execute(&params)
                .context(SqlErrorKind::NonFtsInsertionIntoTempSearchTableFailed)
                .map_err(|e| e.into())
                .map(|_c| ())
        }).collect::<Result<Vec<()>>>();
    ...
    fn insert_fts_searches<'a>(&self, entities: &'a [ReducedEntity<'a>], search_type: SearchType) -> Result<()> {
        ...
        let results: Result<Vec<()>> = chunks.into_iter().map(|chunk| -> Result<()> {
            ...
            stmt.execute(&fts_params).context(SqlErrorKind::FtsInsertionFailed)?;
            ...
            // TODO: consider ensuring we inserted the expected number of rows.
            let mut stmt = self.prepare_cached(s.as_str())?;
            stmt.execute(&params).context(SqlErrorKind::FtsInsertionIntoTempSearchTableFailed)
                .map_err(|e| e.into())
                .map(|_c| ())
        }).collect::<Result<Vec<()>>>();
        ...
        stmt.execute(&[]).context(SqlErrorKind::FtsFailedToDropSearchIds)?;
        results.map(|_| ())
    }
...
pub fn update_partition_map(conn: &rusqlite::Connection, partition_map: &PartitionMap) -> Result<()> {
    ...
    stmt.execute(&params[..]).context(SqlErrorKind::FailedToUpdatePartitionMap)?;
    Ok(())
}
